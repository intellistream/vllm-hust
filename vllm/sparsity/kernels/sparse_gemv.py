...
def _reject_sparse_action(reason: str) -> None:
    """Fail closed in an explicitly strict sparse-dispatch run."""
    if _requires_backend_kernel():
        raise RuntimeError(
            "Strict sparse dispatch rejected before native action: " + reason
        )
    return None
...
def _try_ascend_direct_t_fast_path(
    ...
    if weight_t is None:
        return _reject_sparse_action("transposed weight is unavailable")
    if not _is_npu_tensor(x):
        return _reject_sparse_action("input is not an NPU tensor")
    if not _same_supported_sparse_dtype(x, weight_t):
        return _reject_sparse_action("input and weight dtype/identity mismatch")
    if x.dim() != 2 or weight_t.dim() != 2:
        return _reject_sparse_action("input and weight must both be rank 2")
    if x.shape[1] != weight_t.shape[0]:
        return _reject_sparse_action("input and weight hidden shapes do not match")
    if not _threshold_numel_is_supported(threshold, x):
        return _reject_sparse_action("threshold shape does not match input rows")

    op = _get_ascend_direct_t_op()
    if op is None:
        return _reject_sparse_action(
            "activation_sparse_linear_direct_t ABI is unavailable"
        )
...
def _try_ascend_silu_and_mul_direct_t_fast_path(
    ...
    if weight_t is None:
        return _reject_sparse_action("fused transposed weight is unavailable")
    if not _is_npu_tensor(x):
        return _reject_sparse_action("fused input is not an NPU tensor")
    if not _same_supported_sparse_dtype(x, weight_t):
        return _reject_sparse_action("fused input and weight dtype/identity mismatch")
    if x.dim() != 2 or weight_t.dim() != 2:
        return _reject_sparse_action("fused input and weight must both be rank 2")
    if x.shape[1] != weight_t.shape[0] or weight_t.shape[1] % 2 != 0:
        return _reject_sparse_action("fused gate/up shape contract is invalid")
    if not _threshold_numel_is_supported(threshold, x):
        return _reject_sparse_action("fused threshold shape does not match input rows")

    op = _get_ascend_silu_and_mul_direct_t_op()
    if op is None:
        return _reject_sparse_action(
            "activation_sparse_silu_and_mul_direct_t ABI is unavailable"
        )
...
def sparse_gemv_topk_matmul_silu_impl(
    ...
    if not _is_npu_tensor(x):
        return _reject_sparse_action("top-k input is not an NPU tensor")
    if not _same_supported_sparse_dtype(x, weight_t):
        return _reject_sparse_action("top-k input and weight dtype/identity mismatch")
    if x.dim() != 2 or x.shape[0] != 1:
        return _reject_sparse_action("top-k path requires a single rank-2 row")
    if weight_t.dim() != 2 or x.shape[1] != weight_t.shape[0]:
        return _reject_sparse_action("top-k input and weight shapes do not match")
    if weight_t.shape[1] % 2 != 0:
        return _reject_sparse_action("top-k gate/up output width must be even")
    if keep <= 0 or keep > x.shape[1]:
        return _reject_sparse_action("top-k keep count is outside the hidden size")
...
def sparse_gemv_direct_t_cached_impl(
    ...
    *,
    ...
    if not _is_npu_tensor(x):
        return _reject_sparse_action("cached input is not an NPU tensor")
    if not _same_supported_sparse_dtype(x, weight_t):
        return _reject_sparse_action("cached input and weight dtype/identity mismatch")
    if threshold.dtype != torch.float32 or threshold.device != x.device:
        return _reject_sparse_action("cached threshold dtype/device drifted")
    if not x.is_contiguous() or not weight_t.is_contiguous():
        return _reject_sparse_action("cached input or weight layout drifted")
    if not threshold.is_contiguous():
        return _reject_sparse_action("cached threshold layout drifted")
    op = _get_ascend_direct_t_op()
    if op is None:
        return _reject_sparse_action(
            "cached activation_sparse_linear_direct_t ABI is unavailable"
        )
