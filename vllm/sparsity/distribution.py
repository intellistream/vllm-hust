...
class SparsifyFn(nn.Module):
    ...
    def try_apply_linear(
        ...
        if not self.use_sparse_gemv:
            return None
        if x.dim() != 2:
            return _reject_sparse_action("linear input must be rank 2")
        if not self._sparse_linear_applies_to_all_rows(x):
            return _reject_sparse_action(
                "linear request mixes eligible and ineligible rows"
            )

        weight = getattr(linear_layer, "weight", None)
        if weight is None or not weight.is_contiguous():
            return _reject_sparse_action("linear weight is missing or non-contiguous")
        ...
        if getattr(linear_layer, "quant_config", None) is not None:
            return _reject_sparse_action("quantized linear ABI is unsupported")
        if not self._should_use_sparse_linear_kernel(x, weight):
            return _reject_sparse_action("shape or sparsity policy rejected the kernel")

        if not self._tensor_model_parallel_world_size_is_one():
            return _reject_sparse_action("tensor-parallel identity is not one")
        ...
        if not can_use_sparse_gemv(
            ...
            return _reject_sparse_action(
                "linear dtype or runtime contract is unsupported"
            )

        sparse_input = self._sparse_linear_input(x, linear_layer)
        if sparse_input is None:
            return _reject_sparse_action("sparse input identity is unavailable")

        threshold, inclusive = self._sparse_linear_threshold(sparse_input)
        if threshold is None:
            return _reject_sparse_action("sparse threshold identity is unavailable")
    ...
    def try_apply_gate_up_silu(
        ...
        if not self.use_sparse_gemv:
            return None
        if x.dim() != 2:
            return _reject_sparse_action("fused linear input must be rank 2")
        if not self._sparse_linear_applies_to_all_rows(x):
            return _reject_sparse_action(
                "fused request mixes eligible and ineligible rows"
            )

        weight = getattr(linear_layer, "weight", None)
        if weight is None or not weight.is_contiguous():
            return _reject_sparse_action(
                "fused linear weight is missing or non-contiguous"
            )
        if weight.dim() != 2 or weight.shape[0] % 2 != 0:
            return _reject_sparse_action("fused gate/up weight shape is invalid")
        ...
        if getattr(linear_layer, "quant_config", None) is not None:
            return _reject_sparse_action("quantized fused linear ABI is unsupported")
        if not self._should_use_sparse_linear_kernel(x, weight):
            return _reject_sparse_action(
                "fused shape or sparsity policy rejected the kernel"
            )

        if not self._tensor_model_parallel_world_size_is_one():
            return _reject_sparse_action("fused tensor-parallel identity is not one")
        ...
        if not can_use_sparse_gemv(
            ...
            return _reject_sparse_action(
                "fused dtype or runtime contract is unsupported"
            )

        sparse_input = self._sparse_linear_input(x, linear_layer)
        if sparse_input is None:
            return _reject_sparse_action("fused sparse input identity is unavailable")
        ...
        threshold, inclusive = self._sparse_linear_threshold(sparse_input)
        if threshold is None:
            return _reject_sparse_action("fused threshold identity is unavailable")
        ...
        if output is None:
            return _reject_sparse_action("fused native action returned no output")
