...
class RequestOwnedKVStore:
    ...
    def extend(self, command: OwnerCommand) -> AllocationResult:
        ...
        # ``num_computed_tokens`` is the verified logical watermark.  A
        # rejected speculative suffix remains physically reserved and is
        # overwritten by the next step, so allocation must extend from the
        # physical watermark rather than allocate that suffix twice.
        num_new = command.required_num_tokens - record.reserved_num_tokens
        if num_new <= 0:
            return self._accepted_allocation(command, record, None)
        blocks = self._allocate(
            record.allocator_id,
            record.num_prompt_tokens,
            record.reserved_num_tokens,
            num_new,
            command.required_num_tokens,
        )
        if blocks is None:
