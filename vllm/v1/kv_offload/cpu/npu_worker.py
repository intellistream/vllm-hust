...
class AscendSingleDirectionOffloadingHandler:
    """Handles one NPU<->CPU transfer direction using Ascend batch copy."""
    ...
    def transfer_async(
        ...
        for group_size, block_idx, group_data_refs in zip(
            ...
            dst_blocks_count = (
                dst_logical_blocks_count + self.dst_block_size_factor - 1
            ) // self.dst_block_size_factor
            dst_end_offset = dst_offset + dst_blocks_count
            assert dst_end_offset <= num_dst_blocks

            src_blocks_count = (
                src_logical_blocks_count + self.src_block_size_factor - 1
        ...
        if self.npu_to_cpu:
            stream.wait_stream(npu.current_stream())
        if self._transfers:
        ...
        with npu.stream(stream):
            ...
            if num_copy_ops > 0:
                # vllm-ascend custom op: 0 = CPU->NPU, 1 = NPU->CPU.
...
class AscendCPUOffloadingWorker(OffloadingWorker):
    """OffloadingWorker for Ascend NPU <-> CPU KV cache transfers."""
    ...
    def __init__(
        ...
        _get_torch_npu()
        logger.info(
            "Allocating %d CPU tensors for Ascend offload...",
    ...
    def get_finished(self) -> list[TransferResult]:
        return (
            self._store_handler.get_finished()
            + self._load_handler.get_finished()
        )
