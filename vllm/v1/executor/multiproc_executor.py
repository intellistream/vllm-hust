...
class WorkerProc:
    ...
    def enqueue_output_batch(
        ...
        if len(outputs) <= 1:
            ...
            return
        first = outputs[0]
        if any(not first.can_batch_synchronize_with(output) for output in outputs[1:]):
        ...
        try:
            # Events in one synchronization domain are recorded in stream order,
            # so waiting for the last event makes earlier outputs ready as well.
        ...
        while True:
            ...
            while len(batch) < self.host_batching_max_outputs:
                ...
                if not isinstance(candidate, AsyncModelRunnerOutput):
                    ...
                    batch = []
                    break
                if not batch[0].can_batch_synchronize_with(candidate):
                    queue_wait_us = (time.perf_counter() - wait_start) * 1_000_000
                    self.enqueue_output_batch(batch, queue_wait_us)
                    self.enqueue_output(candidate)
                    batch = []
