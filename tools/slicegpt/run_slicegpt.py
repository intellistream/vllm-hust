...
def slicing_arg_parser(interactive: bool = True) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--skip-final-eval",
        action="store_true",
        help="Skip final PPL evaluation after saving the sliced model.",
    )
    parser.add_argument(
        "--ppl-only",
        action="store_true",
...
def process_slicing_args(args):
    ...
    if not 0 <= args.sparsity < 1:
        raise argparse.ArgumentTypeError("Sparsity should be in the range [0, 1)")
    ...
    if args.dtype == "fp16":
        ...
        raise argparse.ArgumentTypeError(
            "Data type should be one of 'fp16', 'bf16', 'fp32'"
        )
...
def slicing_main(args: argparse.Namespace) -> None:
    ...
    if args.sliced_model_path or args.ppl_only:
        ...
        dataset_ppl = gpu_utils.evaluate_ppl(
            model, model.config.pad_token_id, test_loader
        )
    ...
    rotate.rotate_and_slice(
        model_adapter, train_loader, scheduler, final_orientation=args.final_orientation
    )
    ...
    if args.save_dir:
        ...
        sliced_model_name = (
            sliced_model_dir / f"{pathlib.Path(args.model).name}_{args.sparsity}.pt"
        )
    ...
    if args.skip_final_eval:
        logging.info("Skipping final PPL evaluation after rotating and slicing")
    else:
        reset_model_device()
        dataset_ppl = gpu_utils.evaluate_ppl(
            model, model.config.pad_token_id, test_loader
    ...
    logging.info(
        f"Sliced model parameters: {sliced_param_count:,d} (sliced fraction {sliced_fraction:.4f})"
    )
