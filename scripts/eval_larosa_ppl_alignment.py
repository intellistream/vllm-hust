...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "La RoSA calibration root containing histograms/layer-{idx}/self_attn/D.pt."
        ),
    )
    parser.add_argument(
        "--larosa-repo-path",
    ...
    def _install_hooks(self) -> None:
        ...
        for layer_idx, layer in enumerate(layers):
            ...
            if self._targets("mlp.down"):
                self.handles.append(
                    layer.mlp.down_proj.register_forward_pre_hook(self._make_h2_hook())
                )
...
def run_hf_reference(
    ...
    return BackendComparison(
        backend=(
            f"hf_larosa:{reference_name}:prefill_sparsify={args.vllm_prefill_sparsify}"
        ),
        dense=dense,
        sparse=sparse,
    )
    ...
    if args.json_output:
        payload = {
            "setup": {
                ...
                "vllm_sparse_gemv_min_sparsity": (args.vllm_sparse_gemv_min_sparsity),
