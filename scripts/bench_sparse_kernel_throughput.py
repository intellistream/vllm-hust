...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Restrict activation sparsity to this zero-based layer id. Can be repeated."
        ),
    )
    parser.add_argument(
...
def build_prompts(args: argparse.Namespace) -> list[dict[str, list[int]]]:
    ...
    if text_prompt_ids is not None:
        if len(text_prompt_ids) < args.input_len:
            repeats = (args.input_len + len(text_prompt_ids) - 1) // len(
                text_prompt_ids
            )
...
def main() -> int:
    ...
    if args.sparse_gemv_min_sparsity is not None:
        os.environ["VLLM_SPARSE_GEMV_MIN_SPARSITY"] = str(args.sparse_gemv_min_sparsity)
    ...
    sparse_marker_pids = sorted(
        {int(record["pid"]) for record in sparse_marker_records if "pid" in record}
    )
    ascend_marker_pids = sorted(
        {int(record["pid"]) for record in ascend_marker_records if "pid" in record}
    )
    ascend_marker_ops = sorted(
        {str(record["op"]) for record in ascend_marker_records if "op" in record}
    )
    sparse.update(
        {
    ...
    result = {
        ...
        "sparse_gemv_dense_fallback_policy": (args.sparse_gemv_dense_fallback_policy),
        "sparse_gemv_min_sparsity": active_min_sparsity,
        "require_sparse_gemv_kernel": not args.allow_sparse_gemv_fallback,
        "keep_sparse_markers_during_timing": (args.keep_sparse_markers_during_timing),
