    ...
    if args.dataset_name:
        ...
        if args.dataset_size is not None:
            ...
            if getattr(args, "dataset_sample", "first") == "random":
                dataset = dataset.shuffle(seed=getattr(args, "dataset_seed", 0)).select(
                    range(size)
                )
            else:
...
def run_hf_reference(
    ...
    if args.hf_reference_impl == "official_repo":
        if args.target_projection:
            raise ValueError(
                "--target-projection is supported only with --hf-reference-impl hooks."
            )
...
def score_vllm_forced_decode_next_token_windows(
    ...
    for window in tqdm(windows, desc=desc):
        ...
        for output, target in zip(outputs, targets):
            ...
            if item is None:
                raise KeyError(
                    f"Target token id {target} missing from forced decode logprobs."
                )
            nll -= float(item.logprob)
            count += 1
...
def summarize_sparse_kernel_markers(
    ...
    summary: dict[str, Any] = {
        ...
        "sparse_gemv_marker_pids": sorted(
            {int(record["pid"]) for record in sparse_records if "pid" in record}
        ),
        ...
        "ascend_sparse_linear_marker_pids": sorted(
            {int(record["pid"]) for record in ascend_records if "pid" in record}
        ),
        "ascend_sparse_linear_marker_ops": sorted(
            {str(record["op"]) for record in ascend_records if "op" in record}
        ),
...
def validate_required_sparse_kernel_markers(
    ...
    if not any(
        op in accepted_ops for op in marker_summary["ascend_sparse_linear_marker_ops"]
    ):
        raise RuntimeError(
