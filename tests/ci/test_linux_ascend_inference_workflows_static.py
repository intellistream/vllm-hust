...
def test_pr_inference_workflows_checkout_with_https_without_ssh_known_hosts():
    for workflow_path in WORKFLOW_PATHS:
        ...
        assert (
            "TARGET_REPO_URL: https://github.com/${{ github.repository }}.git" in text
        )
        ...
        env_block = text[text.index("    env:") : text.index("    steps:")]
...
def test_inference_workflows_remove_stale_torch_packages_and_verify_import():
    for workflow_path in WORKFLOW_PATHS:
        ...
        verify_step = text[
            text.index("      - name: Verify installation") : text.index(
                "      - name: Run real"
            )
        ]
...
def test_inference_workflows_install_matching_ascend_torch_stack():
    for workflow_path in WORKFLOW_PATHS:
        ...
        if "ensure_ascend_torch_stack.sh" not in text:
            assert '"$PYTHON_BIN" -c "import torch, torch_npu;' in text
            assert (
                '"$PYTHON_BIN" -m pip install "torch==2.9.0" '
                '"torch-npu==2.9.0"' in text
            )
...
def test_inference_workflows_fetch_target_sha_without_default_branch_clone():
    for workflow_path in WORKFLOW_PATHS:
        ...
        assert ('fetch --no-tags --depth 1 origin "$target_ref"') in checkout_step
...
def test_inference_workflows_cleanup_tolerates_checkout_failure():
    for workflow_path in WORKFLOW_PATHS:
        ...
        assert (
            '[[ -f "$VLLM_ASCEND_HUST_REPO/scripts/use_single_ascend_env.sh" ]]'
            in cleanup_step
        )
        assert (
            '[[ -f ".github/workflows/scripts/cleanup_ascend_ci_processes.sh" ]]'
            in cleanup_step
        )
