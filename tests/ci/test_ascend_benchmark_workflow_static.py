...
def test_issue_comment_path_uses_pr_head_sha_and_base_sha():
    ...
    assert (
        ...
        "needs.issue-comment-command.outputs.pr_head_sha || github.sha }}"
    ) in text
    assert (
...
def test_issue_comment_path_keeps_publish_secrets_disabled():
    ...
    assert (
        ...
        "secrets.VLLM_ASCEND_HUST_BENCHMARK_SSH_KEY"
    ) in text
...
def test_issue_comment_help_is_posted_without_self_hosted_runner():
    ...
    assert (
        "`/ascend smoke`, `/ascend group ...`, `/ascend scenario ...`, and "
        "`/ascend official ...` are planned but not supported yet."
    ) in text
