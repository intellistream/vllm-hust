...
class RecordingBackend:
    ...
    def transfer_async(self, job_id, transfer_spec):
        ...
        is_store = isinstance(src_spec, GPULoadStoreSpec)
        operation = "store" if is_store else "load"
        self.events.append((f"backend_submit_{operation}", job_id, src_spec, dst_spec))
        return self.submit_store_result if is_store else self.submit_load_result
...
def test_transfer_job_remains_backward_compatible():
    ...
    assert job.transfer_spec == (src_spec, dst_spec)
...
def test_disabled_worker_path_preserves_load_semantics():
    ...
    assert events == [
        ("backend_submit_load", 7, *job.transfer_spec),
    ]
...
def test_deferred_store_keeps_full_job_and_submits_before_load():
    ...
    assert worker._unsubmitted_store_jobs == [(3, store_job.transfer_spec)]
    assert events == []
    ...
    assert backend_calls == [
        ("backend_submit_store", 3, *store_job.transfer_spec),
        ("backend_submit_load", 7, *load_job.transfer_spec),
    ]
