...
B134 KV-tiering benchmark JSONL event sink, implemented as an out-of-tree
vLLM plugin.
...
- **serialization** — typed events are mapped to the historical B134 JSONL
  format, preserving event names and field semantics so existing analysis
...
export B134_EVENTS_FILE=/path/to/b134_events.jsonl
...
Line-buffered JSONL, one event per line:

```json
{"event":"transfer_submit","fields":{...},"pid":...,"request_id":"job3","ts_monotonic_ns":...}
```

Event names: `admission`, `scheduled`, `preempt`, `wakeup`, `cpu_store`,
`cpu_evict`, `restore_start`, `restore_done`, `evict`, `sched_step`,
`swap_d2h_submit`, `gather_h2d`, `transfer_submit`, `copy_observed_complete`.
